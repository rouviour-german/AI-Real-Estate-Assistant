...
Examples:
  ./scripts/docker/gpu-internet.sh              # Full featured mode
  ./scripts/docker/gpu-internet.sh --dry-run    # Show commands
...
echo "Starting AI Real Estate Assistant in Docker GPU+Internet mode..."
...
python3 "$PROJECT_ROOT/scripts/start.py" --mode docker --docker-mode gpu --internet "$@"
